                        ...
                        <tr>
                            ...
                            <td>
                                @if($dayEntries->has('punch_in'))
                                    <span class="badge p-2 text-bg-success" style="cursor: pointer;" onclick="editTimeEntry('{{ $dayEntries['punch_in']->id }}', 'punch_in', '{{ $dayEntries['punch_in']->entry_time->format('H:i') }}', '{{ $date }}')">{{ $dayEntries['punch_in']->entry_time->format('h:i A') }}</span>
                                @else
                            ...
                            <td>
                                @if($dayEntries->has('lunch_start'))
                                    <span class="badge p-2 text-bg-warning" style="cursor: pointer;" onclick="editTimeEntry('{{ $dayEntries['lunch_start']->id }}', 'lunch_start', '{{ $dayEntries['lunch_start']->entry_time->format('H:i') }}', '{{ $date }}')">{{ $dayEntries['lunch_start']->entry_time->format('h:i A') }}</span>
                                @else
                            ...
                            <td>
                                @if($dayEntries->has('lunch_end'))
                                    <span class="badge p-2 text-bg-warning" style="cursor: pointer;" onclick="editTimeEntry('{{ $dayEntries['lunch_end']->id }}', 'lunch_end', '{{ $dayEntries['lunch_end']->entry_time->format('H:i') }}', '{{ $date }}')">{{ $dayEntries['lunch_end']->entry_time->format('h:i A') }}</span>
                                @else
                            ...
                            <td>
                                @if($dayEntries->has('punch_out'))
                                    <span class="badge p-2 text-bg-danger" style="cursor: pointer;" onclick="editTimeEntry('{{ $dayEntries['punch_out']->id }}', 'punch_out', '{{ $dayEntries['punch_out']->entry_time->format('H:i') }}', '{{ $date }}')">{{ $dayEntries['punch_out']->entry_time->format('h:i A') }}</span>
                                @else
                                    @if($dayEntries->has('punch_in'))
                                        <span class="badge p-2 text-bg-warning" style="cursor: pointer;" onclick="editTimeEntry('', 'punch_out', '', '{{ $date }}')">Still Working</span>
                                    @else
                            ...
                            <td>
                                @if($dayEntries->has('holiday'))
                                    <span class="badge p-2 text-bg-secondary">{{ $dayEntries['holiday']->notes }}</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
