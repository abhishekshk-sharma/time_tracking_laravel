...
<head>
    <meta charset="utf-8">
    <title>Payslip - {{ $employee->username }}</title>
    <style>
        /* GOOGLE STYLE RESET */
        @page { margin: 0px; }
        body {
        ...
        .text-gray { color: #5f6368; } /* Google Muted Gray */
        .text-blue { color: #1a73e8; } /* Google Blue */
        ...
        .attendance-stat { display: inline-block; width: 22%; text-align: center; }
...
<body>
    ...
    <table class="header-table">
        <tr>
            <td width="60%">
                <img src="data:image/png;base64,{{ base64_encode(file_get_contents(public_path('images/logo.png'))) }}" style="height: 40px; margin-bottom: 10px;">
            ...
            <td width="40%" style="vertical-align: bottom;" class="text-right">
                ...
                <div class="text-gray" style="margin-top: 5px;">{{ $month }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="emp-info-table" style="margin-bottom: 20px;">
        <tr>
            ...
            <td class="emp-value">{{ strtoupper($employee->username) }}</td>
            <td class="emp-label">Department</td>
            <td class="emp-value">{{ $employeeDepartment }}</td>
        </tr>
        <tr>
            <td class="emp-label">Employee ID</td>
            <td class="emp-value">{{ $employee->emp_id }}</td>
            <td class="emp-label">Designation</td>
            <td class="emp-value">{{ $employee->designation ?? 'N/A' }}</td>
        </tr>
        <tr>
            <td class="emp-label">Date of Joining</td>
            <td class="emp-value">{{ $employee->hire_date }}</td>
            <td class="emp-label">Bank Account</td>
            <td class="emp-value">************1234</td> </tr>
    </table>
        ...
        <table width="100%">
            <tr>
                ...
                <td class="text-center" style="border-left: 1px solid #dfe1e5;">
                    ...
                    <span class="stat-value text-blue">{{ $payableDays }}</span>
                </td>
                <td class="text-center" style="border-left: 1px solid #dfe1e5;">
                    <span class="stat-label">Leaves Taken</span>
                    <span class="stat-value">{{ $casualLeave + $sickLeave }}</span>
                </td>
                <td class="text-center" style="border-left: 1px solid #dfe1e5;">
                    <span class="stat-label">LWP</span>
                    <span class="stat-value">{{ $absentDays ?? 0 }}</span>
                </td>
            </tr>
        </table>
    </div>
    ...
    <table class="salary-table" style="margin-top: 20px;">
        <thead>
            <tr>
                ...
                <th width="5%"></th> <th width="25%">Deductions</th>
                <th width="15%" class="text-right">Amount</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>Basic Salary</td>
                <td class="text-right">{{ number_format($calculatedBasicSalary, 2) }}</td>
                <td></td>
                <td>Professional Tax</td>
                <td class="text-right">{{ number_format($salary->pt, 2) }}</td>
            </tr>
            <tr>
                ...
                <td class="text-right">{{ number_format($salary->pf, 2) }}</td>
            </tr>
            <tr>
                <td>Conveyance Allowance</td>
                <td class="text-right">{{ number_format($salary->conveyance_allowance, 2) }}</td>
                <td></td>
                <td></td>
                <td></td>
            </tr>
            <tr>
                ...
                <td class="text-right">0.00</td> <td></td>
                <td></td>
                <td></td>
            </tr>
            <tr class="last-row">
            ...
            <tr style="background-color: #fff;">
                ...
                <td class="bold text-right">₹ {{ number_format($grossEarnings, 2) }}</td>
                <td></td>
                <td class="bold text-gray">TOTAL DEDUCTIONS</td>
                <td class="bold text-right">₹ {{ number_format($totalDeductions, 2) }}</td>
            </tr>
        </tbody>
    </table>
