...
<div class="page-header">
    ...
    <div class="page-options">
        <button type="button" class="btn btn-outline-primary" onclick="openCTCModal()">
            <i class="fas fa-calculator"></i> Auto Edit Salary
        </button>
    </div>
</div> --}}
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                ...
                <div class="form-group">
                    ...
                    <select name="payment_mode" id="payment_mode" class="form-control" required>
                        <option value="bank_transfer" {{ old('payment_mode', $salary->payment_mode) == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
...
function calculateComponents() {
    ...
    $.ajax({
        url: '{{ route("admin.calculate-salary-components") }}',
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
...
function applyCalculation() {
    // alert('Applying calculated values to the form '+ window.calculatedData.components.basic_salary);
    console.log(' this is calculated data');
    if (window.calculatedData.components) {
